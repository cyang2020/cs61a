...
def free_bacon(opponent_score):
	...
	# BEGIN PROBLEM 2
	remainder = opponent_score % 10
	num = opponent_score // 10
	return max (remainder, num) + 1
...
def is_perfect_piggy(turn_score):
	...
	n = sqrt(turn_score) // 1
	b = (turn_score ** (1. / 3)) // 1
	if n * n == turn_score or b ** 3 == turn_score:
		return True
	else:
		return False
...
def is_swap(score0, score1):
	...
	# BEGIN PROBLEM 5
	if score0 > score1:
		return is_swap(score1, score0)
	return score1 == score0 * 2
	# END PROBLEM 5
...
def max_scoring_num_rolls(dice=six_sided, num_samples=1000):
	...
	max_average_score = averaged_roll_dice(num_rolls, dice)
	max_scoring_num_rolls = 1
	for num_rolls in range(1, 11):
		if averaged_roll_dice(num_rolls, dice) > max_average_score:
			max_average_score = averaged_roll_dice(num_rolls, dice)
...
def swap_strategy(score, opponent_score, margin=8, num_rolls=4):
	...
	if is_prime(a):
		a = next_prime(a)
	if (a + score) * 2 == opponent_score or a > margin:
		return 0
...
def final_strategy(score, opponent_score):
	...
	*** YOUR DESCRIPTION HERE ***
	...
	#1  if i can use free_bacon to win the game:
	if free_bacon(opponent_score) + score >= 100:
		return 0
	#2 when to play agrasivly and when to play conservetivly
	elif score >= 88:
		return 4

	elif (opponent_score - score) >= 85:
		return 9
	elif (opponent_score - score) >= 80:
		return 8
	elif (opponent_score - score) >= 75:
		return 7
	elif (opponent_score - score) >= 70:
		return 6

	if 2 * (score + 1) == opponent_score:
		return  swap_strategy(score, opponent_score, 15, 10)
	if 2 * opponent_score == score + 1:
		return swap_strategy(score, opponent_score, 20,0)

	if 2 * opponent_score == score + free_bacon(opponent_score):
		return swap_strategy(score, opponent_score, 20,10)
