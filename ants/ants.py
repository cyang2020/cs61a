		...
		if insect.is_ant:
			if self.ant is insect:
				...
				if hasattr(self.ant, 'container') and self.ant.container and self.ant.ant is insect:
					if hasattr(self.ant.ant, 'impostor'):
...
class ThrowerAnt(Ant):
	...
	def nearest_bee(self, hive):
		...
		"""
		# BEGIN Problem 5
		thisplace = self.place
		distence = 0
		while thisplace.entrance != None or thisplace is not hive:
			if len(thisplace.bees) > 0 and distence >= self.min_range and distence <= self.max_range:
				return random_or_none(thisplace.bees)
			else:
				thisplace = thisplace.entrance
				distence += 1
		# END Problem 5
...
class FireAnt(Ant):
	...
	def reduce_armor(self, amount):
		...
		"""
		# BEGIN Problem 4
		flamezone = self.place
		if self.armor <= amount:
			for bee in flamezone.bees[:]:
...
class NinjaAnt(Ant):
	...
	def action(self, colony):
		# BEGIN Problem 8
		ninjazone = self.place
		for bee in ninjazone.bees[:]:
			bee.reduce_armor(self.damage)
				# END Problem 8
...
class HungryAnt(Ant):
	...
	def eat_bee(self, bee):
		...
		self.progress = self.time_to_digest
		# END Problem 10
...
class QueenAnt(ScubaThrower):  # You should change this line
	...
	def action(self, colony):
		if self.impostor:
			...
			beside = self.place.exit
			while beside != None:
				if beside.ant != None:
					if beside.ant not in self.SuperSaiyanBee:
						beside.ant.damage *= 2
						self.SuperSaiyanBee.append(beside.ant)
					if beside.ant.container and beside.ant.ant != None and beside.ant.ant not in self.SuperSaiyanBee:
						beside.ant.ant.damage *= 2
						self.SuperSaiyanBee.append(beside.ant.ant)
				beside = beside.exit
			self.throw_at(self.nearest_bee(colony.hive))
...
def apply_effect(effect, bee, duration):
	...
	# BEGIN Problem EC
	thisaction = effect(bee.action)
	bee.action = thisaction(duration)
	# END Problem EC
