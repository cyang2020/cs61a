...
def find_centroid(cluster):
	...
	# BEGIN Question 5
	lat = mean([restaurant_location(i)[0] for i in cluster])
	lon = mean([restaurant_location(i)[1] for i in cluster])
	return [lat, lon]
	# END Question 5
...
def best_predictor(user, restaurants, feature_fns):
	...
	for i in feature_fns:
		answer += [find_predictor(user, reviewed, i)]
	return max(answer, key = lambda x : x[1])[0] # find the max r2(x[1]) and return the answer([0])
	# END Question 8
...
def rate_all(user, restaurants, feature_fns):
	...
	for restaurant in restaurants:
		disname = restaurant_name(restaurant)
		if restaurant in reviewed:
			answer[disname] = user_rating(user, disname)
		else:
			answer[disname] = predictor(restaurant)
	return answer
