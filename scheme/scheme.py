...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        opperation = scheme_eval(first, env)
        check_procedure(opperation)
        return opperation.eval_call(rest, env)
        # END PROBLEM 5
...
def eval_all(expressions, env):
    ...
    # BEGIN PROBLEM 8
    "*** REPLACE THIS LINE ***"
    if expressions == nil: #(begin)
        return None
    elif not isinstance(expressions, Pair):#(begin 1)
        return scheme_eval(expressions, env)
    elif isinstance(expressions, Pair) and expressions.second != nil: #(begin 1 2 3)
        scheme_eval(expressions.first, env)
        return eval_all(expressions.second, env) #recursively calling the function to solve the problem
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        alloperan = operands.map(lambda x: scheme_eval(x, env))
        return scheme_apply(self, alloperan, env)
        # END PROBLEM 5
...
class LambdaProcedure(UserDefinedProcedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 12
        env = env.make_child_frame(self.formals, args)
        env.parent = self.env#new frames's parent should be current frame for lambda expression
        return env
        # END PROBLEM 12
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        lambda_val = LambdaProcedure(target.second, expressions.second, env)
        env.define(target.first, lambda_val)
        return target.first
        # END PROBLEM 10
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        bind = bindings.first
        check_form(bind, 2, 2)#check individual bind structure
        formals = Pair(bind.first, formals)
        vals =  Pair(scheme_eval(bind.second.first, env), vals)
        bindings = bindings.second
    if check_formals(formals):#check all the formals
        raise SchemeError
...
def scheme_call_cc(function, env):
    class ContinuationError(Exception):
        ...
        def __init__(self, value):
            ...
            self.value = value
    try:
        a = ContinuationProcedure(ContinuationError)
        c = Pair(a, nil)
        complete_eval(scheme_apply(function, c, env))
    except ContinuationError as b:
        return b.value
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if not scheme_listp(expr):
            raise SchemeError('malformed list: {0}'.format(str(expr)))
        first, rest = expr.first, expr.second #first = the first intake(which is the the opp)
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            result = SPECIAL_FORMS[first](rest, env)
        else:
            # BEGIN PROBLEM 20 first = opp second = what is evaluated

            expression = scheme_eval(first, env) #first use eval to find opperation #then find the arguments
            check_procedure(expression)
            result = expression.eval_call(rest, env)
            # END PROBLEM 20
    return result
